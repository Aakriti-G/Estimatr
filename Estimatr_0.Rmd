...
Below, we have presented an example using the 'wage1' package from the 'wooldrigde'package. We are interested in analyzing the effect of education of an employee on the wages they earn. The standard errors of this regression on R are obtained using the lm() function as so:
```{r}
library(wooldridge)
...
+ lm_robust  
This function is used to get the robust standard errors from a linear regression model. Below we have used the same example as the one before but this time we try to get robust standard errors.
...
For lm_robust we care about 2 important arguments: se_type and clusters.
