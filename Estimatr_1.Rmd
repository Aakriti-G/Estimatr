...
The `wage2` data set is found in the wooldridge R package.It has 935 observations on 17 variables,some of which are:
...
However the lm()function is designed to fit linear models which assume homoscedasticity. This assumption states that the variance of the error terms is the same across all the independent variables. If this assumption is violated the regression estimates remain unbiased and consistent but they are no longer efficient. And because of the inconsistency of the covariance matrix of the estimated regression coefficients, the tests of hypotheses (t-test, f-test) are no longer valid. Therefore, it is important to check if the assumption for homoscedasticity holds for our regression.
...
An instrumental variable is a variable, Z, used in regression analysis when you have endogenous variables i.e. variables that are influenced by other variables in the model. The Instrumental Variables (IV) regression controls for threats to internal validity such as confounding variables, measurement error, omitted variable bias (spuriousness), simultaneity and reverse causality. A good instrument is the one which is correlated with the independent variable, X but not with the error term, i.e. it affects the outcome variable,Y only through X. 

In the previous example, we aim to show whether the level of education of an employee affects their wage. However, we fail to account for other variables, such as the IQ of the employee which is a potential confounding variable, meaning that it may affect both education and wage, and ignoring it in the analysis may lead to omitted variable bias. Therefore, we can include IQ as an instrument variable in a 2-Stage Least Squares (2SLS) analysis of the relationship between wage and education. 
